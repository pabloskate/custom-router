...
describe("routeAndProxy", () => {
  ...
  it("auto-describes latest user images before sending a routed text-only model", async () => {
    const secret = "1234567890abcdef";
    const defaultApiKeyEnc = await encryptByokSecret({
      plaintext: "gateway-default-key",
      secret,
    ...
    upstreamMock
      .mockResolvedValueOnce({
        ...
        response: new Response(
          JSON.stringify({
            choices: [{ message: { content: "The screenshot shows a disabled checkout button." } }],
          }),
          { status: 200, headers: { "content-type": "application/json" } }
        ),
      })
        ...
        profiles: [
          {
            ...
            models: [
              ...
              { gatewayId: "gw_default", modelId: "model/text", name: "Text", modality: "text->text" },
            ],
            defaultModel: key("gw_default", "model/text"),
        ...
        gatewayRows: [
          {
            ...
            models: [
              ...
              { id: "model/text", name: "Text", modality: "text->text" },
              { id: "model/vision-sidecar", name: "Vision Sidecar", modality: "text,image->text" },
            ],
          },
        ],
      },
    ...
    expect(result.response.status).toBe(200);
    expect(classifierMock).toHaveBeenCalledWith(expect.objectContaining({
      imageDescriptionAvailable: true,
    }));
    expect(upstreamMock).toHaveBeenCalledTimes(2);
    expect(upstreamMock.mock.calls[0]?.[0]).toEqual(expect.objectContaining({
      apiPath: "/chat/completions",
      payload: expect.objectContaining({
        model: "model/vision-sidecar",
      }),
    }));

    const textPayload = upstreamMock.mock.calls[1]?.[0]?.payload as any;
    expect(textPayload.model).toBe("model/text");
    expect(textPayload.messages[0].content).toEqual([
      { type: "text", text: "What is wrong here?" },
      {
        type: "text",
        text: expect.stringContaining("The screenshot shows a disabled checkout button."),
      },
    ]);
    expect(JSON.stringify(textPayload)).not.toContain("image_url");
    expect(repository.putExplanation).toHaveBeenCalledWith(
      expect.objectContaining({
        explanation: expect.objectContaining({
          notes: expect.arrayContaining([
            "Latest user image input was converted to text for text-only model model/text using vision model model/vision-sidecar.",
          ]),
        }),
      })
    );
  ...
  it("keeps native image payloads when the routed model supports image input", async () => {
    const secret = "1234567890abcdef";
    const defaultApiKeyEnc = await encryptByokSecret({
      plaintext: "gateway-default-key",
      secret,
    ...
    repositoryMock.mockReturnValue(repository as any);
    classifierMock.mockResolvedValue({
      selectedModel: "model/native-vision",
      confidence: 0.91,
      signals: ["frontier_classification"],
    });
    upstreamMock.mockResolvedValue({
        ...
        profiles: [
          {
            ...
            models: [
              { gatewayId: "gw_default", modelId: "model/classifier", name: "Classifier" },
              { gatewayId: "gw_default", modelId: "model/native-vision", name: "Vision", modality: "text,image->text" },
            ],
            defaultModel: key("gw_default", "model/native-vision"),
            classifierModel: key("gw_default", "model/classifier"),
          },
        ],
        gatewayRows: [
          {
            ...
            models: [
              { id: "model/classifier", name: "Classifier" },
              { id: "model/native-vision", name: "Vision", modality: "text,image->text" },
              { id: "model/vision-sidecar", name: "Vision Sidecar", modality: "text,image->text" },
            ],
          },
        ],
