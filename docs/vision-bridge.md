...
The self-hosted endpoint accepts HTTPS image URLs and `data:image/...` base64 URLs. Local file paths must go through the local helper first. Data URLs are capped per image and across the combined request body so large screenshots fail before being forwarded upstream.
